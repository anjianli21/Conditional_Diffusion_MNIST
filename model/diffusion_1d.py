...
class ResidualConvBlock(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        if self.is_res:
            ...
            # TODO: why there is a 1.414 here?
            return out / 1.414
        else:
            x1 = self.conv1(x)
            x2 = self.conv2(x1)
...
class ContextUnet1D(nn.Module):
    def __init__(self, in_channels, n_feat=256, context_dim=5):
        ...
        self.to_vec = nn.Sequential(nn.AvgPool1d(5), nn.GELU())
        ...
        self.up0 = nn.Sequential(
            ...
            nn.ConvTranspose1d(2 * n_feat, 2 * n_feat, 5),  # otherwise just have 2*n_feat
    ...
    def forward(self, x, c, t, context_mask):
        ...
        # c = nn.functional.one_hot(c, num_classes=self.context_dim).type(torch.float)

        # mask out context if context_mask == 1
        # context_mask = context_mask[:, None]
        # context_mask = context_mask.repeat(1, self.context_dim)
        # context_mask = (-1 * (1 - context_mask))  # need to flip 0 <-> 1
        # c = c * context_mask
        # TODO: in previous code, the c is in negative sign, don't know the reason
        c[context_mask == 1] = 0
...
class DDPM1D(nn.Module):
    ...
    def sample(self, n_sample, size, device, guide_w=0.0):
        ...
        c_i = torch.randn(n_sample, 5).to(device)  # context for us just cycles throught the mnist labels

        # don't drop context at test time
        context_mask = torch.zeros(c_i.size()[0]).to(device)
        c_i = c_i.repeat(2, 1) # double the batch
